...
YouTube 스트림 추출이 깨질 때마다 NewPipe Extractor 최신 버전으로 흡수하는 절차와 누적 지식. **현재 SubFeed가 의존하는 버전**: `gradle/libs.versions.toml`의 `newpipe = "v0.26.3"`.
...
### 2026-06 — SABR 강제로 360p 고정 + chunk 403 + 라이브 파싱 실패

- **증상**: 360p muxed 고정, 고화질 추출 시 chunk 403/EOF, 라이브 방송 파싱 실패 — 전반적으로 "문제가 굉장히 많은" 상태
- **원인**: YouTube가 ANDROID/IOS 클라이언트에 SABR(Server-side Adaptive BitRate) 강제 적용 — URL 기반 adaptive 스트림이 막히고 `serverAbrStreamingUrl`만 반환됨 (TeamNewPipe/NewPipe#13320)
- **NewPipe 해결**: v0.26.3 — SABR이 아직 강제되지 않은 visionOS 클라이언트를 스트림 소스로 추가 (PR #1508). visionOS 스트림 URL은 PoToken 없이도 chunk fetch 동작. DASH/HLS manifest만 있는 라이브도 StreamInfo 생성 허용.
- **SubFeed 흡수**: `fix/newpipe-v0.26.3-2026-06` — `libs.versions.toml` 한 줄, API breaking change 없음. 상세 분석: [`20260610_newpipe-v0.26.3-sabr-fix-analysis.md`](20260610_newpipe-v0.26.3-sabr-fix-analysis.md)
- **다음 깨짐 예측**: YouTube가 SABR을 visionOS로 확대하면 동일 증상 재발. made-for-kids/unlisted 영상은 visionOS 미지원으로 기존 경로 폴백.
...
- [ ] 현재 NewPipe 버전 확인: `grep newpipe gradle/libs.versions.toml` → 현재 `v0.26.3`
